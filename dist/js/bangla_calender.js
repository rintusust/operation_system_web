...
var getDigitBanglaFromEnglish = function (str) {
    var retStr = str;
...
function buildCal(m, y, cM, cH, cDW, cD, brdr) {
    ...
    var t = '<div class="' + cM + '"><table class="' + cM + '" cols="7" cellpadding="0" border="' + brdr + '" cellspacing="0"><tr align="center"><td colspan="2">' +
        beng_bc_month_name[bcal1[1]] + '</td><td colspan="3">' + byy + '</td><td colspan="2">' +
        beng_bc_month_name[bcal[1]] + '</td><tr align="center">';
    ...
    for (i = 1; i <= 42; i++) {
        ...
        var indcal1 = indcal[2] == 1 ? '<font size="3" color="#800000">' + convert(indcal[2]) +
            '</font>' : convert(indcal[2])
...
var mas_len = [0, 30.92569444, 62.63289352, 94.00184028, 125.4761458, 156.4885417,
    186.9247338, 216.8066667, 246.3155787, 275.6427546, 305.0935301, 334.9103588, 365.2587564814815];
...
function oneDay() {
    ...
    var str = convert(bcal[2]) + " " + beng_bc_month_name[bcal[1]] + " " +
        convert((bcal[0])) + " বঙ্গাব্দ, " + bWeekbc_days[bar] + "বার।";
    return str;
}
...
function formSubmit() {
    ...
    bdcal += '<tr valign="top">';
    bdcal += '<td>' + buildCal(1, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(2, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(3, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '</tr>';
    bdcal += '<tr valign="top">';
    bdcal += '<td>' + buildCal(4, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(5, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(6, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '</tr>';
    bdcal += '<tr valign="top">';
    bdcal += '<td>' + buildCal(7, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(8, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(9, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '</tr>';
    bdcal += '<tr valign="top">';
    bdcal += '<td>' + buildCal(10, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(11, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '<td>' + buildCal(12, myyear, "bc_main", "bc_month",
        "bc_bc_daysofweek", "bc_days", 1) + '</td>';
    bdcal += '</tr>';
    bdcal += '</table>';
...
function convertToBanglaDate(year, month, day) {
    // var english_number = "" + Bangla_Date(year, month, day + 1); // 1 is added for bangladeshi calender
    var english_number = "" + Bangla_Date(parseInt(year), parseInt(month), parseInt(day));
    var dateText = english_number.split(',');
    mon = beng_bc_month_name[parseInt(dateText[1]) + ""];
    year = getDigitBanglaFromEnglish(dateText[0]);
    day = getDigitBanglaFromEnglish(dateText[2]);
    var bangla_converted_number = getDigitBanglaFromEnglish(english_number);
    return day + " " + mon + " " + year;
}