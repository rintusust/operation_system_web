...
use App\modules\HRM\Models\RequestLoginUser;
use App\modules\HRM\Models\UserLoginSuccessLog;
...
class UserController extends Controller
{
    ...
    public function manageUserInfoDetails(Request $request)
    {
        ...
        $data = CustomQuery::getManageUserList($offset, $limit, $request->q);

        return $this->exportDataCustom(collect($data['users'])->chunk(2000)->toArray(), 'HRM::export.manage_user_report', '', 'user_list');
...
//            }
            ...
            } else return Redirect::to('/operation');
        } else {
    ...
    {
        $users = User::where('visible',1)->count();
...
//            return $e->getMessage();
    ...
    function OtpPasswordRequest(Request $request)
    {
        ...
        if($status=="success"){
            ...
            // if (Session::has('redirect_url')) {
            UserLoginSuccessLog::create([
                'user_id' => session('user_id'),
                'otp_generated_time' =>Carbon::now(),
                'ip' =>request()->ip()
            ]);
    ...
    public function viewActionLog(Request $request, $id = null)
    {
		...
		//DB::enableQueryLog();
                ...
                $data = $user->actionLog()->with('getAnsar');
                ...
                if($request->ansar_id != ''){
                    $data = $data->where('ansar_id', $request->ansar_id);
                }

                $data = $data->select('ansar_id', 'from_state', 'to_state', 'action_type', DB::raw('DATE_FORMAT(created_at,"%d %b. %Y") as date'), DB::raw('DATE_FORMAT(created_at,"%r") as time'))->orderBy('created_at', 'desc')->get();
