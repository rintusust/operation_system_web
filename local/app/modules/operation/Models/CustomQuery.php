...
class CustomQuery {
    ...
    public static function getUserInformation($limit, $offset, $q) {
        ...
        /* $users = DB::connection('operation')->table('tbl_user')
          ->leftJoin('sessions', 'sessions.user_id', '=', 'tbl_user.id')
          ->join('tbl_user_details', 'tbl_user_details.user_id', '=', 'tbl_user.id')
          ->join('tbl_user_log', 'tbl_user_log.user_id', '=', 'tbl_user.id')
    ...
    public static function getNotVerifiedChunkAnsar($limit, $offset, $division = null, $unit = null, $thana = null, $from_ansar = null, $to_ansar = null) {
        DB::enableQueryLog();
        $ansar = DB::table('tbl_vdp_ansar_info')
            ->join('tbl_division', 'tbl_vdp_ansar_info.division_id', '=', 'tbl_division.id')

            ->join('tbl_units', 'tbl_vdp_ansar_info.unit_id', '=', 'tbl_units.id')
                        ->join('tbl_thana', 'tbl_vdp_ansar_info.thana_id', '=', 'tbl_thana.id')
                        ->join('tbl_vdp_designation', 'tbl_vdp_ansar_info.designation', '=', 'tbl_vdp_designation.id')
                        ->where(function ($query) {
                            $query->where('tbl_vdp_ansar_info.status', 'new');
                        })->orderBy('tbl_vdp_ansar_info.id', 'asc');
        if ($division && $division != 'all') {
            $ansar->where('tbl_vdp_ansar_info.division_id', $division);
        }
        if ($unit && $unit != 'all') {
            $ansar->where('tbl_vdp_ansar_info.unit_id', $unit);
        }
        if ($thana && $thana != 'all') {
            $ansar->where('tbl_vdp_ansar_info.thana_id', $thana);
        }
        if ($from_ansar && $to_ansar) {
            $ansar->whereBetween('tbl_vdp_ansar_info.id', [min([$from_ansar, $to_ansar]), max([$from_ansar, $to_ansar])]);
        }
        $b = $ansar->skip($offset)->take($limit)
            ->select('tbl_vdp_ansar_info.id','tbl_vdp_ansar_info.geo_id', 'tbl_vdp_ansar_info.ansar_name_bng as name', 'tbl_vdp_designation.designation_name_bng as rank','tbl_units.unit_name_bng as unit_name', 'tbl_units.unit_code', 'tbl_division.division_name_bng as division_name','tbl_division.division_code','tbl_thana.thana_name_bng as thana_name',  'tbl_vdp_ansar_info.profile_pic', 'tbl_vdp_ansar_info.date_of_birth','tbl_vdp_ansar_info.sign_pic')
                ->get();
    ...
    public static function getNotVerifiedAnsar($limit, $offset, $sort = "desc", $division = null, $unit = null, $thana = null, $type) {
        ...
        $ansar = DB::table('tbl_vdp_ansar_info')
            ...
            ->join('tbl_units', 'tbl_vdp_ansar_info.unit_id', '=', 'tbl_units.id')
                ...
                ->join('tbl_vdp_designation', 'tbl_vdp_ansar_info.designation', '=', 'tbl_vdp_designation.id')
                ->where(function ($q) {
            $q->where('tbl_vdp_ansar_info.status', 'new');
        });
        ...
        if ($type == 'view') {
            $b = $ansar->skip($offset)->take($limit)->orderBy('tbl_vdp_ansar_info.id', $sort)
                    ->select('tbl_vdp_ansar_info.id','tbl_vdp_ansar_info.geo_id', 'tbl_vdp_ansar_info.ansar_name_bng as name', 'tbl_vdp_designation.designation_name_bng as rank', 'tbl_units.unit_name_bng as unit_name', 'tbl_units.unit_code', 'tbl_division.division_name_bng as division_name','tbl_division.division_code','tbl_thana.thana_name_bng as thana_name', 'tbl_vdp_ansar_info.profile_pic', 'tbl_vdp_ansar_info.date_of_birth','tbl_vdp_ansar_info.sign_pic')
...
//            throw new Exception();
    ...
    public static function disembodimentEntry(Request $request) {
        ...
        if (auth()->user()->type == 11 || auth()->user()->type == 77) {
            ...
            $rules['memorandum_id'] = 'required|unique:operation.tbl_memorandum_id,memorandum_id|unique:operation.tbl_embodiment,memorandum_id|unique:operation.tbl_rest_info,memorandum_id||unique:operation.tbl_transfer_ansar,transfer_memorandum_id';
        }
