...
class DGController extends Controller
{
        ...
        try {
            ...
            if (!$ansarEmbodimentInfo->exists()) {
                $ansarEmbodimentInfo = DB::table('tbl_embodiment_log')
                    ...
                    ->select('tbl_embodiment_log.joining_date', 'tbl_embodiment_log.old_memorandum_id as memorandum_id', 'tbl_kpi_info.kpi_name', 'tbl_units.unit_name_bng', 'tbl_thana.thana_name_bng')->first();$ansarCurrentEmbodimentInfo = [];
                $ansarCurrentEmbodimentInfo = [];

            } else {
                $ansarEmbodimentInfo = $ansarEmbodimentInfo
                    ->select('tbl_embodiment.*','tbl_embodiment.joining_date', 'tbl_embodiment.memorandum_id as memorandum_id', 'tbl_kpi_info.kpi_name', 'tbl_units.unit_name_bng', 'tbl_thana.thana_name_bng')
                    ->first();

                // Check For Current KPI Data
                $ansarCurrentEmbodimentInfo = $ansarEmbodimentInfo;
                if($ansarEmbodimentInfo->transfered_date == $ansarEmbodimentInfo->joining_date){
                    $ansarCurrentEmbodimentInfo = $ansarEmbodimentInfo;
                }else{
                    $transfer_data = TransferAnsar::where('embodiment_id', $ansarEmbodimentInfo->id)->orderBy('id','desc')->first();
                    $ansarCurrentEmbodimentInfo->memorandum_id = $transfer_data->transfer_memorandum_id;


                }


                if(is_null($ansarEmbodimentInfo->joining_kpi_id)){
                    $kpi_data = $this->checkJoiningKPI($ansarEmbodimentInfo);
                    if($kpi_data){
                        $ansarEmbodimentInfo->kpi_name = $kpi_data->kpi_name;
                        $ansarEmbodimentInfo->unit_name_bng = $kpi_data->unit_name_bng;
                        $ansarEmbodimentInfo->thana_name_bng = $kpi_data->thana_name_bng;
                    }
                }else{
                    $kpi_data = DB::table('tbl_kpi_info')
                        ->join('tbl_units', 'tbl_units.id', '=', 'tbl_kpi_info.unit_id')
                        ->join('tbl_thana', 'tbl_thana.id', '=', 'tbl_kpi_info.thana_id')
                        ->where('tbl_kpi_info.id', $ansarEmbodimentInfo->joining_kpi_id)
                        ->select('tbl_kpi_info.*', 'tbl_units.unit_name_bng','tbl_thana.thana_name_bng')->first();
                    $ansarEmbodimentInfo->kpi_name = $kpi_data->kpi_name;
                    $ansarEmbodimentInfo->unit_name_bng = $kpi_data->unit_name_bng;
                    $ansarEmbodimentInfo->thana_name_bng = $kpi_data->thana_name_bng;                }
            }
            $ansarCurrentEmbodimentInfo = [];
            $ansarCurrentEmbodimentInfo = DB::table('tbl_embodiment')
                ->join('tbl_kpi_info', 'tbl_kpi_info.id', '=', 'tbl_embodiment.kpi_id')
                ->join('tbl_units', 'tbl_units.id', '=', 'tbl_kpi_info.unit_id')
                ->join('tbl_thana', 'tbl_thana.id', '=', 'tbl_kpi_info.thana_id')
                ->where('tbl_embodiment.ansar_id', Input::get('ansar_id'));

            if ($ansarCurrentEmbodimentInfo->exists()) {
                $ansarCurrentEmbodimentInfo = $ansarCurrentEmbodimentInfo
                    ->select('tbl_embodiment.*','tbl_embodiment.joining_date', 'tbl_embodiment.memorandum_id as memorandum_id', 'tbl_kpi_info.kpi_name', 'tbl_units.unit_name_bng', 'tbl_thana.thana_name_bng')
                    ->first();
            }
            ...
            return json_encode(['apid' => $ansarPersonalDetail, 'api' => $ansarPanelInfo, 'aod' => $ansarOfferInfo, 'aoci' => $offer_cancel, 'asi' => $ansarStatusInfo,
                'aei' => $ansarEmbodimentInfo, 'acei' => $ansarCurrentEmbodimentInfo, 'adei' => $ansarDisEmbodimentInfo, 'status' => $ansarStatusInfo->getStatus()[0], "offer_zone" => $a]);
        } catch (\Exception $e) {
            return [$e->getTraceAsString()];
        }
    }

    function checkJoiningKPI($data){
            if($data->transfered_date == $data->joining_date){
              return [];
            }else{
               $kpi_id =  $this->foundKpiJoiningDate($data->id);
               $kpi_data = DB::table('tbl_kpi_info')
                   ->join('tbl_units', 'tbl_units.id', '=', 'tbl_kpi_info.unit_id')
                   ->join('tbl_thana', 'tbl_thana.id', '=', 'tbl_kpi_info.thana_id')
    ...
    function foundKpiJoiningDate($embodied_id){
        $data = TransferAnsar::where('embodiment_id', $embodied_id)->orderBy('id','asc')->first();
        return $data->present_kpi_id;
    }
...
//            CustomQuery::addDGlog(['ansar_id' => $request->input('ansar_id'), 'action_type' => 'EMBODIED', 'from_state' => $status, 'to_state' => 'EMBODIED']);
    ...
    function directDisEmbodimentSubmit(Request $request)
    {
        ...
        try {
            ...
            $memorandum_entry = new MemorandumModel();
            $memorandum_entry->memorandum_id = $request->mem_id;
            $memorandum_entry->mem_date = Carbon::now()->format('Y-m-d');
            $memorandum_entry->save();
