...
use App\modules\HRM\Models\CorrectionLog;
use Carbon\Carbon;
    ...
    public function newEmbodimentEntry(Request $request)
    {
        ...
        foreach ($request->ansar_ids as $ansar_id) {
            ...
            try {
				...
				$max_joining_date =  Carbon::parse($sms_receive_info->sms_received_datetime)->addDay(7);
...
//            return $sms_receive_info->offered_district!=$request->division_name_eng?"same":"differ";
				...
				// Added By Rintu - Instructed By ANSAR ICT (07/08/2022)

                if (Carbon::parse($request->reporting_date)->gt($max_joining_date)) {
                    throw new \Exception('Reporting date greater then 
                    max reporting date for Ansar ID: ' . $ansar_id . " .max reporting date " . $max_joining_date->format('d-m-Y'));
                }

                if (Carbon::parse($request->reporting_date)->lt(Carbon::parse($sms_receive_info->sms_received_datetime))) {
                    throw new \Exception('Reporting date less then 
                    offer accept date for Ansar ID: ' . $ansar_id . " .offer accept date " . $sms_receive_info->sms_received_datetime);
                }
...
//        return Input::get('transferred_ansar');
        ...
        $error_data = ['message'=> 'Joining Date cannot be back date'];
        
        if(Carbon::parse($t_date)->lt(Carbon::now())){
             return response(json_encode($error_data), 400, ['Content-Type' => 'application/json']);
        }
            ...
            foreach ($transferred_ansar as $ansar) {
                ...
                try {
                    ...
                    if ($e_id) {
                        ...
                        $transfer->current_rank = $e_id->ansar->designation_id;
   ...
   public function disembodimentReasonCorrectionView()
    {
        return view('HRM::Embodiment.disembodiment_reason_correction_view');
    }

    public function loadAnsarForDisembodimentReasonCorrection()
    {
        ...
        $ansar_details = DB::table('tbl_rest_info')
            ...
            ->select('tbl_rest_info.ansar_id as id', 'tbl_disembodiment_reason.reason_in_bng as disembodiment_reason', 'tbl_rest_info.disembodiment_reason_id', 'tbl_ansar_parsonal_info.ansar_name_eng as name', 'tbl_ansar_parsonal_info.sex', 'tbl_ansar_parsonal_info.data_of_birth as dob', 'tbl_designations.name_eng as rank', 'tbl_units.unit_name_eng as unit', 'tbl_thana.thana_name_eng as thana')
            //->orderBy('r_date', 'desc')
            ->first();
        return Response::json(['ansar_details' => $ansar_details,'reasons' => $reasons]);
    }

    public function newDisembodimentReasonEntry(Request $request)
    {
        $rules = [
            'ansar_id' => 'required|numeric|regex:/^[0-9]+$/|exists:tbl_embodiment_log,ansar_id',
            'disembodiment_reason' => ['required|numeric|regex:/^[0-9]+$/| exists:tbl_disembodiment_reason,id'],
            'memorandum_id' => ['required']
        ];
        $message = [
            'ansar_id.required' => 'Ansar ID is required',
            'disembodiment_reason.required' => 'New Disembodiment Reason is required',
            'ansar_id.numeric' => 'Ansar ID must be numeric',
            'ansar_id.regex' => 'Ansar ID must be numeric',
            'disembodiment_reason.regex' => 'New Disembodiment Reason format is invalid',
            'memorandum_id.required' => 'Memorandum ID is required',        ];
        ...
        $ansar_id = $request->input('ansar_id');
        $modified_new_disembodiment_reason = $request->input('dis-reason');
        $memo_id = $request->input('memorandum_id');
        //echo $modified_new_disembodiment_reason;exit;
        DB::beginTransaction();
        try {
            ...
            $embodiment_log_update->disembodiment_reason_id = $modified_new_disembodiment_reason;
            $embodiment_log_update->save();

            $rest_info_update = RestInfoModel::where('ansar_id', $ansar_id)->first();
            $previous_data = $rest_info_update->disembodiment_reason_id;
            if ($rest_info_update) {
                $rest_info_update->disembodiment_reason_id = $modified_new_disembodiment_reason;
                $rest_info_update->save();
            }
            CorrectionLog::insert([
                'ansar_id' => $ansar_id,
                'type' => 'disembodiment_reason_correction',
                'memo_id' => $memo_id,
                'user_id' => auth()->user()->id,
                'corresponding_id' => $rest_info_update->id,
                'previous_data' => $previous_data
            ]);
            CustomQuery::addActionlog(['ansar_id' => $ansar_id, 'action_type' => 'DISEMBODIMENT REASON CORRECTION', 'from_state' => 0, 'to_state' => 0, 'action_by' => auth()->user()->id]);
            DB::commit();
        } catch (\Exception $e) {
            return Redirect::back()->with('error_message', $e->getMessage());
        }
        return Redirect::route('disembodiment_reason_correction_view')->with('success_message', 'Dis-Embodiment Reason is corrected Successfully!');
    }
            ...
            foreach ($data as $ansar) {
                ...
                try {
                    ...
                    if ($e_ansar) {
                        ...
                        $transfer->current_rank = $ansar->$e_ansar->designation_id;
