    ...
    <script>
        GlobalApp.controller('NewEmbodimentController', function ($scope, $http, notificationService, $timeout, $rootScope) {
            ...
            $scope.addToCart = function () {
                ...
                // Check reporting date & joining date difference check - Rintu Kumar Chowdhury (14/12/2021)//

                var reporting_date = moment($scope.reporting_datee); //reporting date
                var joining_date = moment($scope.joining_datee); // joining date
                var duration = moment.duration(joining_date.diff(reporting_date));
                var days = duration.asDays();
                console.log(days)

                if(days > 30){
                    notificationService.notify("error", "embodiment date must be within 1 month after reporting date")
                    return;
                }
